export type Product = {
  id: number;
  slug: string;
  name: string;
  description: string;
  primaryImage: string;
};

export const PRODUCTS: Product[] = [
  {
    id: 1,
    slug: 'adeio-portofoli-tshirt-anergia-szn',
    name: 'xrostao άδειο πορτοφόλι t-shirt anergia season',
    description: 'Αυτό που μας ενέπνευσε να φτιάξουμε αυτό το t-shirt είναι μια καθημερινή εικόνα που βλέπουμε κάθε φορά που κοιτάμε άμα μπορούμε να αγοράσουμε κάτι.',
    primaryImage: '/images/tshirts/1/main.png',
  },
  {
    id: 2,
    slug: 'epidoma-tshirt-anergia-season',
    name: 'xrostao επίδομα t-shirt anergia season',
    description: 'Απλά πάρε επίδομα ανεργίας, είναι δωρεάν χρήματα!!',
    primaryImage: '/images/tshirts/2/main.png',
  },
  {
    id: 3,
    slug: 'agrios-san-tigris-tshirt-anergia-season',
    name: 'xrostao άγριος σαν τίγρης t-shirt anergia season',
    description: 'Αυτό το t-shirt είναι για σκληρούς τίγρεις.',
    primaryImage: '/images/tshirts/3/main.png',
  },
  {
    id: 4,
    slug: 'ena-fili-tshirt-anergia-season',
    name: 'xrostao ένα φιλί t-shirt anergia season',
    description: 'Κάποια/ος σας χρωστάει ένα φιλί ή της/του χρωστάτε εσείς.',
    primaryImage: '/images/tshirts/4/main.png',
  },
  {
    id: 5,
    slug: 'ftino-faghto-tshirt-anergia-season',
    name: 'xrostao φθηνό φαγητό t-shirt anergia season',
    description: 'Αντί να φας κάτι πάρε αυτή την μπλούζα.',
    primaryImage: '/images/tshirts/5/main.png',
  },
  {
    id: 6,
    slug: 'tape-tshirt-anergia-season',
    name: 'xrostao tape t-shirt anergia season',
    description: 'Δεν υπάρχει εδώ description απλά έχει tape.',
    primaryImage: '/images/tshirts/6/main.png',
  },
  {
    id: 7,
    slug: 'pagkaki-tshirt-anergia-season',
    name: 'xrostao παγκάκι t-shirt anergia season',
    description: 'Φόρα αυτή την μπλούζα και πήγαινε και κάτσε σε ένα παγκάκι, αυτό.',
    primaryImage: '/images/tshirts/7/main.png',
  },
  {
    id: 8,
    slug: 'arabiko-tshirt-anergia-season',
    name: 'xrostao αραβικό t-shirt anergia season',
    description: 'Τα αραβικά γράμματα είναι fire απλά για αυτό υπάρχει αυτό το t-shirt.',
    primaryImage: '/images/tshirts/8/main.png',
  },
  {
    id: 9,
    slug: 'gimno-baddie-tshirt-anergia-season',
    name: 'xrostao γυμνό baddie t-shirt anergia season',
    description: 'Από billionaire σε millionaire.',
    primaryImage: '/images/tshirts/9/main.png',
  },
  {
    id: 10,
    slug: 'astegos-tshirt-anergia-season',
    name: 'xrostao άστεγος t-shirt anergia season',
    description: 'Άνεργος με κόκκινα γράμματα για να δηλώσουμε πως η ανεργία έχει βαρέσει κόκκινο.',
    primaryImage: '/images/tshirts/10/main.png',
  },
  {
    id: 11,
    slug: 'gamw-tous-logariasmous-tshirt-anergia-season',
    name: 'xrostao γαμώ τους λογαριασμούς t-shirt anergia season',
    description: 'Απλά ένα t-shirt που όταν το φορέσεις θα νιώσεις πιο ξέγνοιαστος για τα χρέη σου.',
    primaryImage: '/images/tshirts/11/main.png',
  },
  {
    id: 12,
    slug: 'expired-idol-tshirt-anergia-season',
    name: 'xrostao expired idol tshirt',
    description: 'Κάποτε θεωρούνταν 10άρι, ενώ πλέον όχι, οπότε γαμάει η ειρωνεία, στα ρούχα',
    primaryImage: '/images/tshirts/12/main.png',
  },
  {
    id: 13,
    slug: 'greek-traditions-tshirt-anergia-season',
    name: 'xrostao greek traditions tshirt',
    description: 'Πώς καταλαβαίνεις ότι γαμάει να είσαι στην Ελλάδα',
    primaryImage: '/images/tshirts/13/main.png',
  },
  {
    id: 14,
    slug: 'following-the-trends-tshirt',
    name: 'xrostao following the trends tshirt',
    description: 'Μην παιρνεις τα παπουτσια που ειναι στην μοδα δεν θα ξεχωριζουν απο τα αλλα',
    primaryImage: '/images/tshirts/14/main.png',
  },
  {
    id: 15,
    slug: 'anakyklosi-tshirt',
    name: 'xrostao anakyklosi tshirt',
    description: 'Αρχισε να ανακυκλωνεις πετοντας τα παλια σου tshirt στους μωβ καδους και παρε καινουρια απο εδω',
    primaryImage: '/images/tshirts/15/main.png',
  },
  {
    id: 16,
    slug: 'mhn-kapnizeis-tshirt',
    name: 'xrostao mhn kapnizeis tshirt',
    description: 'Σοβαρα μην καπνιζεις!',
    primaryImage: '/images/tshirts/16/main.png',
  },
  {
    id: 17,
    slug: 'chein-tshirt',
    name: 'xrostao chein tshirt',
    description: 'Το chain το εγραψα επιτηδες chein απλα σκεψου και θα καταλαβεις γιατι',
    primaryImage: '/images/tshirts/17/main.png',
  },
  {
    id: 18,
    slug: '26510-tshirt',
    name: 'xrostao 26510 tshirt',
    description: 'Αποκλειστικα για πολιτες Ιωννιτων',
    primaryImage: '/images/tshirts/18/main.png',
  },
  {
    id: 19,
    slug: 'gyros-tshirt',
    name: 'xrostao gyros tshirt',
    description: 'Απλα κολλαει με το culture για αυτο το εβαλα',
    primaryImage: '/images/tshirts/19/main.png',
  },
  {
    id: 20,
    slug: 'blured-tshirt',
    name: 'xrostao blured tshirt',
    description: 'Το να βαζεις blur γαμαει',
    primaryImage: '/images/tshirts/20/main.png',
  },
  {
    id: 21,
    slug: 'akako-oplo-tshirt',
    name: 'xrostao akako oplo tshirt',
    description: 'Αμα δεν προοθεις βια , θα πετυχεις',
    primaryImage: '/images/tshirts/21/main.png',
  },
  {
    id: 22,
    slug: 'roloi-tshirt',
    name: 'xrostao roloi tshirt',
    description: 'Για να τημησεις την πολη σου και την ιστορια της , παρε αυτο το tshirt',
    primaryImage: '/images/tshirts/22/main.png',
  },
  {
    id: 23,
    slug: '26510-night-life',
    name: 'xrostao 26510 night life',
    description: 'Μην πινεις αλκοολ',
    primaryImage: '/images/tshirts/23/main.png',
  },
  {
    id: 24,
    slug: 'kastro-tshirt',
    name: 'xrosta kastro tshirt',
    description: 'Απλά ένα ακόμα xrostao t-shirt.',
    primaryImage: '/images/tshirts/24/main.png',
  },
  {
    id: 25,
    slug: 'tshirt-25',
    name: 'xrostao leo oti thelo tshirt',
    description: 'Φορώντας το t-shirt αυτό, απλά λες ό,τι νιώθεις.',
    primaryImage: '/images/tshirts/25/main.png',
  },
  {
    id: 26,
    slug: 'fyllo-26510-tshirt',
    name: 'xrostao fyllo 26510 tshirt',
    description: "don't use weapons",
    primaryImage: '/images/tshirts/26/main.png',
  },
  {
    id: 27,
    slug: 'xortari-tshirt',
    name: 'xrostao xortari tshirt',
    description: 'για οικολογους παρολο που το tshirt δεν ειναι απο οικολογικα υλικα',
    primaryImage: '/images/tshirts/27/main.png',
  },
  {
    id: 28,
    slug: 'spider-tshirt',
    name: 'xrostao spider tshirt',
    description: 'μπρο δεν εισαι ο spider man',
    primaryImage: '/images/tshirts/28/main.png',
  },
  {
    id: 29,
    slug: 'thoo-tshirt-2',
    name: 'xrostao thoo tshirt 2',
    description: 'ειναι ωραιο το Blur',
    primaryImage: '/images/tshirts/29/main.png',
  },
  {
    id: 30,
    slug: 'rip-tshirt',
    name: 'xrostao rip tshirt',
    description: 'για οσους εχουν φυγει',
    primaryImage: '/images/tshirts/30/main.png',
  },
  {
    id: 31,
    slug: 'summer-season-gynaikeio-magio',
    name: 'xrostao summer season tshirt gynaikeio magio',
    description: 'απλα εσκαγε ωραιο',
    primaryImage: '/images/tshirts/31/main.png',
  },
  {
    id: 32,
    slug: 'summer-season-finikes',
    name: 'xrostao summer season tshirt finikes',
    description: 'για να καθεσε κατω απο φινικες',
    primaryImage: '/images/tshirts/32/main.png',
  },
  {
    id: 33,
    slug: 'summer-season-chill-trapezi',
    name: 'xrostao summer season tshirt chill trapezi',
    description: 'ελπιζω να τρως καλα οπου και να βρισκεσε',
    primaryImage: '/images/tshirts/33/main.png',
  },
  {
    id: 34,
    slug: 'summer-season-sun',
    name: 'xrostao summer season tshirt sun',
    description: 'μου αρεσουν οι ηλιοι οποτε εκανα ενα τετοιο tshirt',
    primaryImage: '/images/tshirts/34/main.png',
  },
  {
    id: 35,
    slug: 'summer-season-mple-filo',
    name: 'xrostao summer season tshirt mple filo',
    description: 'σε ολους αρεσουν τα λουλουδια',
    primaryImage: '/images/tshirts/35/main.png',
  },
  {
    id: 36,
    slug: 'summer-season-potaki',
    name: 'xrostao summer season tshirt potaki',
    description: 'πιες ενα ποτακι φωρωντας το tshirt',
    primaryImage: '/images/tshirts/36/main.png',
  },
  {
    id: 37,
    slug: 'adeia-karta-tshirt',
    name: 'xrostao adeia karta tshirt',
    description: 'Οταν η καρτα ειναι αδεια και δεν εχει αλλα λεφτα',
    primaryImage: '/images/tshirts/37/main.png',
  },
];

export function getProductById(id: number) {
  return PRODUCTS.find((p) => p.id === id);
}

export function getProductBySlug(slug: string) {
  return PRODUCTS.find((p) => p.slug === slug);
}

const MAIN_IMAGE_SCALE_BY_ID: Record<number, number> = {
  1: 1.5,
  2: 1.2,
  3: 1,
  4: 1.03,
  5: 0.97,
  6: 1.05,
  7: 1.01,
  8: 0.98,
  9: 1.01,
  10: 0.92,
  11: 1.2,
};

const DETAIL_IMAGE_SCALE_BY_KEY: Record<string, number> = {
  '1-1': 1.5,
  '1-2': 1.3,
  '1-3': 1.11,
  '1-4': 1.09,
  '2-1': 1.2,
  '2-2': 1.23,
  '2-3': 1.09,
  '2-4': 1.13,
  '3-1': 1,
  '3-2': 1.39,
  '3-3': 1.18,
  '3-4': 0.97,
  '4-1': 1.03,
  '4-2': 0.99,
  '4-3': 1,
  '4-4': 1,
  '5-1': 0.97,
  '5-2': 1.15,
  '5-3': 1,
  '5-4': 1,
  '6-1': 1.05,
  '6-2': 1.06,
  '6-3': 1,
  '6-4': 1,
  '7-1': 1.01,
  '7-2': 0.78,
  '7-3': 1.11,
  '7-4': 1,
  '8-1': 0.98,
  '8-2': 1.13,
  '8-3': 1,
  '8-4': 1,
  '9-1': 1.01,
  '9-2': 1.01,
  '9-3': 1,
  '9-4': 1,
  '10-1': 0.92,
  '10-2': 0.78,
  '10-3': 1.33,
  '10-4': 1.21,
};

export function getProductMainImageScale(id: number) {
  return 1.8;
}

export function getProductDetailImageScale(id: number, imgNum: number) {
  return 1.8;
}
